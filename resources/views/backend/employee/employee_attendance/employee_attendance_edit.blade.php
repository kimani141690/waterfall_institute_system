...
<section class="content">
   ...
   <div class="row">
   	<div class="col-md-6">
		...
		<div class="controls">
	 <input type="date" name="date" class="form-control" value="{{ $edit_data['0']['date'] }}">  
	  </div>		 
	</div>
...
<input name="attendance_status{{$key}}" type="radio" value="Present" id="present{{$key}}" checked="checked" {{ ($data->attendance_status == 'Present')?'checked':'' }} >
 ...
 <input name="attendance_status{{$key}}" value="Leave" type="radio" id="leave{{$key}}"  {{ ($data->attendance_status == 'Leave')?'checked':'' }}  >
 <label for="leave{{$key}}">Leave</label>

 <input name="attendance_status{{$key}}" value="Absent"  type="radio" id="absent{{$key}}"  {{ ($data->attendance_status == 'Absent')?'checked':'' }}  >
