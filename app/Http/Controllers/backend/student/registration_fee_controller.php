...
class registration_fee_controller extends Controller
{
    public function reg_fee_view(){
    ...
    public function reg_fee_course_data(Request $request){
        ...
        foreach ($all_student as $key => $v) {
            $registrationfee = fee_category_amount::where('fee_category_id','1')->where('course_id',$v->course_id)->first();
            $color = 'success';
   ...
   public function reg_fee_payslip(Request $request){
    $student_id= $request->student_id;
    $course_id = $request->course_id;

    $all_student['details'] = assign_student::with(['student','discount'])->where('student_id',$student_id)->where('course_id',$course_id)->first();
